...
// IMPORTANT: Replace YOUR_URL and YOUR_KEY with real values!
// ================================================================

// ⚠️  STEP 1: Replace these with your Supabase project values
...
// Initialize client (using CDN build)
...
const DB = {
    ...
    async searchJobs(query) {
        const { data, error } = await sb
            ...
            .select('*')
            .in('status', ['published', 'featured'])
            .or(`title.ilike.%${query}%,description.ilike.%${query}%,organization.ilike.%${query}%,location.ilike.%${query}%`)
            .order('created_at', { ascending: false });
        if (error) throw error;
        return data || [];
    },

    // Subscribe to job changes (realtime)
    subscribeJobs(callback) {
        return sb.channel('jobs-public')
    ...
    async incrementViews(id) {
        await sb.rpc('increment_views', { job_id: id });
    },
    ...
    async saveJob(jobData, editId = null) {
        if (editId) {
            ...
            const { error } = await sb.from('jobs').insert(jobData);
            if (error) throw error;
        }
    },
...
// Category config used on every page
const CATS = {
...
function toast(msg, type = '') {
    ...
    t.style.cssText = `background:white;border-left:4px solid ${colors[type]||colors['']};border-radius:10px;padding:12px 18px;box-shadow:0 8px 24px rgba(0,0,0,.15);font-size:.9rem;font-weight:500;min-width:240px;animation:toastIn .3s ease;`;
    t.textContent = msg;
    c.appendChild(t);
    setTimeout(() => {
        t.style.opacity = '0';
